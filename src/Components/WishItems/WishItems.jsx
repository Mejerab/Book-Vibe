...
const WishItems = ({wishItem}) => {
    ...
    const { bookId, bookName, author, image, tags, category, rating, publisher, yearOfPublishing, totalPages } = wishItem;

    // const wishItems = getWishData();
    // const readItems = getStoredData();
    // const nothing = [];
    // for (const wish of wishItems) {
    //    readItems.find(item=> item === wish && nothing.push(parseInt(wish)));
    // }
    // const bookDatas = () =>{
    //     fetch('../Books.json')
    //     .then(res=> res.json())
        ...
        <div className='border p-6 flex lg:flex-row flex-col items-center mb-5 rounded-lg'>
            ...
            <div className='w-full'>
                ...
                <div className='mt-6 flex'>
                    ...
                    <button onClick={goToDetails} className='btn -mt-3 py-2 ml-2 bg-[#23BE0A] text-white rounded-full'>Wanna Read</button>
                </div>
            </div>
        </div>
    );
